...
// #define WAKE_UP_RINGBUFFER_SIZE 1024 * 5
...
class t_agent
{
	static void f_load_sound(ALuint a_buffer, const std::string& a_path)
	{
		...
		// 	rate = a_rate;
		// });
		// alBufferData(a_buffer, format, data.data(), data.size(), rate); // Copy mp3 data into AL Buffer 0/1/2/3  
	}
	...
	void f_create()
	{
		...
		{
			// ALuint x;
			// alGenSources(1, &x); // Generate Sources 初始化音源ID 
			// std::shared_ptr<ALuint> source{new ALuint(x), [](auto a_x)
			// {
			// 	alDeleteSources(1, a_x);
			// }};
			// alSourcei(*source, AL_LOOPING, AL_TRUE); //设置音频播放是否为循环播放，AL_FALSE是不循环
			// return [source, sounds = v_sounds + (a_type == "TIMER" ? 0 : 2)](bool a_background)
			// {
			// 	alSourceStop(*source);
			// 	alSourcei(*source, AL_BUFFER, sounds[a_background ? 1 : 0]);  // 附加的buffer的ID, Attach buffer 0/1 to source  
			// 	alSourcef(*source, AL_GAIN, a_background ? 1.0f / 16.0f : 1.0f); //设置音量大小，1.0f表示最大音量。openAL动态调节音量大小就用这个方法
			// 	alSourcePlay(*source); //source播放buffer
			// };
			int type;
			return [a_type, type = a_type == "TIMER" ? 0 : 2](bool a_background)
			{
				std::cout<<"type="<<a_type<<std::endl;
				for(int i=0;i<40;i++)
					mw_playAudio(2,"./sounds/alarm.mp3");
			};
		...
		try {
			...
			v_session->f_alerts_duration(60);
			v_session->f_content_can_play_in_background(false);
			v_session->f_capture_threshold(60999);
			v_session->f_capture_auto(true);
		} catch (std::exception& e) {
			// if (v_log) v_log(e_severity__ERROR) << "loading session/options.json: " << e.what() << std::endl;
		}
		try {
			picojson::value alerts;
			std::ifstream s(FILE_ALERTS_JSON);
			s >> alerts;
			for (auto& x : alerts.get<picojson::value::object>()) 
				v_session->f_alerts_set(x.first, x.second / "type"_jss, x.second / "scheduledTime"_jss);
		} catch (std::exception& e) {
			if (v_log) v_log(e_severity__ERROR) << "loading session/alerts.json: " << e.what() << std::endl;
		}
		try {
			picojson::value speaker;
			std::ifstream s(FILE_SPEAKER_JSON);
			s >> speaker;
	...
	void f_grant(std::map<std::string, std::string>&& a_query, T_done a_done)
	{
		...
		(*http)("POST", a_query)(v_scheduler->f_io(), [this, a_done, http](auto a_socket)
		{
			boost::asio::async_read(*a_socket, http->v_buffer, v_scheduler->wrap([this, a_done, http, a_socket](auto a_ec, auto)
			{
				...
				if (v_log) //v_log(e_severity__TRACE) << "grant: " << http->v_http << ' ' << http->v_code << http->v_message << std::endl << result.serialize(true) << std::endl;
					v_log(e_severity__TRACE) << "grant: " << http->v_code << std::endl;
				if (http->v_code == 200) {
					...
					std::ofstream(FILE_TOKEN) << refresh_token;
					if (!v_session) this->f_create();
					if (!v_session && (wakeup_is_init == false)) {
						t_session* v_se = this->f_session();
						wakeup_is_init = true;
						if(mw_createWakeuper(MEDIAWIN) < 0){
							printf("Can't init asr module.\n");
							return ;
						}
						mw_startWake(t_session::audio_dataReceived, t_session::audioStatusChanged);
						mw_setUserInfo((void*)v_se);
						std::cout<<"wakeup init"<<std::endl;
					}
		...
		}, v_scheduler->wrap([this, a_done](auto a_ec)
		{
			if (v_log) //v_log(e_severity__ERROR) << "grant: " << a_ec.message() << std::endl;
				v_log(e_severity__ERROR) << "grant: ..." << std::endl;
			a_done(a_ec);
		}));
	}
	...
	{
		f_grant({
			{"client_id", v_profile / "client_id"_jss},
...
public:
	...
	t_agent(const std::function<std::ostream&(t_severity)>& a_log, const picojson::value& a_profile, const picojson::value& a_sounds) : v_log(a_log), v_profile(a_profile)
	{
		...
		// v_device = alcOpenDevice(NULL); // 打开一个device, select the "preferred device"  
		// if (v_device == NULL) throw std::runtime_error("alcOpenDevice");
		// v_context = alcCreateContext(v_device, NULL); //创建一个context
		// alcMakeContextCurrent(v_context);
		// alGetError(); // clear error code 
		// alGenBuffers(4, v_sounds);  //创建4个buffer
	...
	~t_agent()
	{
		// alDeleteBuffers(4, v_sounds);  // Unload mp3
		// alcMakeContextCurrent(NULL);
		// alcDestroyContext(v_context);
		// alcCloseDevice(v_device);
		rb_deinit(&g_WakeUpRingBuffer);
	}
	bool f_activated() const
	{
		return static_cast<bool>(std::ifstream(FILE_TOKEN));
	}
	...
	void f_start(boost::asio::io_service& a_io, T_done a_done)
	{
		...
		std::string token;
		std::ifstream(FILE_TOKEN) >> token;
		if (!token.empty()) v_scheduler->dispatch([this, a_done, token]()
		{
			f_create();
			f_refresh(token);

			rb_init(&g_WakeUpRingBuffer, WAKE_UP_RINGBUFFER_SIZE, 0);
			t_session* v_session = f_session();
			if (v_session) {
				wakeup_is_init = true;
				if(mw_createWakeuper(MEDIAWIN) < 0){
					printf("Can't init asr module.\n");
	...
	template<typename T_done>
	void f_grant(string clientId,const std::string& a_code,string redirectUri,string codeVerifier, T_done a_done) //
	{
		v_scheduler->dispatch([this, a_code, a_done, clientId, redirectUri, codeVerifier]()
		{
			this->f_grant({
				{"grant_type", "authorization_code"},
				{"code", a_code},
