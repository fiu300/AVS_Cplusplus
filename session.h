...
extern RingBuffer_t g_WakeUpRingBuffer;

static int wakeup_ready = 0;
...
#define DEB_LINE()  //std::cout<<"===>"<<__FUNCTION__<<" , "<<__LINE__<<std::endl;



static void my_alSourcePause(int source){
	// std::cout<<__FUNCTION__<<" "<<__LINE__<<std::endl;

}

static void my_alSourcef(float value){
	// std::cout<<__FUNCTION__<<" "<<__LINE__<<std::endl;
	char cmd[48] = {0};
	if (value == 1.0)
		sprintf(cmd, "user_i2cBus_control 98 3d %d", 55);
	else if (value == 6.0)
		sprintf(cmd, "user_i2cBus_control 98 3d %d", 55);
	else if (value == 100)
		sprintf(cmd, "user_i2cBus_control 98 3d %d", 50);
	else if(value == 0.0)
		sprintf(cmd, "user_i2cBus_control 98 3d %d", 80);
	system(cmd);
}


static void my_alSourcePlay(unsigned int source){
	// std::cout<<__FUNCTION__<<" "<<__LINE__<<std::endl;
}
...
class t_session
{
	...
	{
		t_task& v_task; //任务
		t_audio_target v_target; // 音频
		std::deque<std::function<void()>> v_directives; //指令队列
		std::string v_playing; //token
...
public:
	...
	class t_alert
	{
		...
		std::string v_type;	//类型
		std::string v_at;   //时间
		std::unique_ptr<boost::asio::system_timer> v_timer; //定时器
...
private:
	...
	struct t_parser
	{
		...
		void f_json_finish()
		{
			...
			if (v_session.v_log) //v_session.v_log(e_severity__TRACE) << "json: " << directive.serialize(true) << std::endl;
				v_session.v_log(e_severity__TRACE) << "get json: " << std::endl;
			...
			if (v_session.v_log) //v_session.v_log(e_severity__INFORMATION) << "parser(" << this << ") directive: " << ns + "." << name << std::endl;
				v_session.v_log(e_severity__INFORMATION) << "parser directive: " << ns + "." << name << std::endl;
			try {
				v_session.v_handlers.at({ns, name})(directive);  //调用相应处理函数
	...
	boost::asio::ssl::context v_tls;	//tls
	t_scheduler& v_scheduler;			//调度器
	nghttp2::asio_http2::header_map v_header; //http2头字段
	std::function<std::function<void(bool)>(const std::string&)> v_open_sound; // 已经捆绑的定时器/闹钟源
	std::unique_ptr<nghttp2::asio_http2::client::session> v_session; //会话控制
	bool v_online = false;
	boost::asio::steady_timer* v_reconnecting = nullptr; //重连定时器
	unsigned int v_reconnecting_interval = 1; //重连间隔
	unsigned int v_message_id = 0;
	unsigned int v_dialog_id = 0;
	bool is_StopCapture = false;

	//处理函数集合
	std::map<std::pair<std::string, std::string>, std::function<void(const picojson::value&)>> v_handlers{
		{{"SpeechRecognizer", "ExpectSpeech"}, [this](auto a_directive) //预期语音
		{
			...
			v_dialog->f_queue([this, dialog_id = a_directive / "directive" / "header" * "dialogRequestId" | std::string(), timeout = static_cast<int>((payload / "timeoutInMilliseconds"_jsn))]()
			{
				...
				{
					this->f_dialog_acquire(*v_recognizer); //获取会话
					...
					v_expecting_timeout = &v_scheduler.f_run_in(std::chrono::milliseconds(timeout), [this](auto a_ec)
					{
						...
						this->f_empty_event("SpeechRecognizer", "ExpectSpeechTimedOut"); //上报状态
						this->f_dialog_release();
						if (v_state_changed) v_state_changed();
					});
					if (v_state_changed) v_state_changed();
				};
				do v_dialog->v_task.f_wait(); while (v_expecting_speech); //等待
			});
		}},
		{{"SpeechRecognizer", "StopCapture"}, [this](auto a_directive)
		{
			is_StopCapture = true;
			if (v_log) v_log(e_severity__INFORMATION) << "StopCapture" <<std::endl;
		}},
	...
	bool v_content_can_play_in_background = true; //false;
	...
	picojson::value f_metadata(const std::string& a_namespace, const std::string& a_name, picojson::value::object&& a_payload)
	{
		if (v_log) //v_log(e_severity__INFORMATION) << "event: " << a_namespace << "." << a_name << std::endl;
			v_log(e_severity__INFORMATION) << "evt: " << "." << a_name << std::endl;
		return picojson::value(picojson::value::object{
			{"event", picojson::value(picojson::value::object{
				{"header", picojson::value(picojson::value::object{
					...
					{"messageId", picojson::value("messateId-4" + std::to_string(++v_message_id))}
				})},
	...
	{
		std::cout<<"exception: "<<a_directive<<" "<<a_type<<" "<<a_message<<std::endl;
	...
	void f_alerts_set(const std::string& a_token, const std::string& a_type, const std::string& a_at)
	{
		...
		i->second.v_timer->async_wait(v_scheduler.wrap([this, i](auto a_ec)
		{
			...
			auto f = [this, i]
			{
				...
				i->second.v_play = v_open_sound(i->second.v_type); //根据type绑定timer或者alert源mp3数据
...
private:
	...
	{
		// alListenerf(AL_GAIN, v_speaker_muted ? 0.0f : v_speaker_volume / 100.0f); //设置音效
		my_alSourcef(v_speaker_muted ? 0.0f : 1.0f);
...
#if 0	
	bool f_capture_back(ALCdevice* a_device, char* a_buffer, bool a_busy)
	{
		while (true) {
	...
	void f_recognizer_back()
	{
		while (true) {

			// std::cout<<"f_recognizer ----- 1 "<<std::endl;

			//初始化捕捉设备用,最后一个参数就决定了sample的最大值。当sample等于设备每次采集的样本数量的最大值时，这些样本数据将会被设备丢弃掉，而我们所要做的就是要将每一个样本数据收集之后进行处理。
			...
			const size_t window = sizeof(buffer) * 100;  // 32k

			// std::cout<<"f_recognizer ----- 2 "<<std::endl;

			while (f_capture(device.get(), buffer, false)) {
				size_t n = deque.size() + sizeof(buffer);
				if (n > window) 
					deque.erase(deque.begin(), deque.begin() + (n - window));
				deque.insert(deque.end(), buffer, buffer + sizeof(buffer));
			}

			// std::cout<<"f_recognizer ----- 3 "<<std::endl;
			...
			// std::cout<<"call f_post size="<<deque.size()<<std::endl;

			auto request = f_post([queue](auto a_p, auto a_n, auto a_flags)
			{
				auto& deque = queue->first;

				// std::cout<<"size="<<deque.size()<<" a_n="<<a_n<<std::endl;
			...
			// std::cout<<"call f_post after"<<std::endl;
			...
			if (request) {
				...
				{
					// std::cout<<"request close"<<std::endl;
...
#endif
	bool f_capture(FILE *fp, char* a_buffer, bool a_busy)
	{
		// std::cout<<"-->%s,%d v_capture_busy=%d,v_capture_auto=%d, v_capture_force=%d\n",__FUNCTION__,__LINE__,v_capture_busy,v_capture_auto,v_capture_force);
		int n = 10;


		if (!a_busy && !v_expecting_timeout && v_expecting_speech) 
			v_expecting_speech();

		if ((v_capture_busy && v_capture_auto && v_dialog->v_playing.empty() && 
			v_content->v_playing.empty() || v_capture_force) != a_busy) {
			// std::cout<<__FUNCTION__<<" "<<__LINE__<<std::endl;
			return false;
		}
	...
	void f_recognizer_2()
	{
		...
		while (true) {
			...
			std::cout<<"----------------------------- start %d------------------------------ " << f_online()<<std::endl;
			...
			FILE *fp = fopen("./req.pcm","r");

			const size_t window = sizeof(buffer) * 100;  // 32k

			// std::cout<<__FUNCTION__<<" "<<__LINE__<<std::endl;

			while (f_capture(fp, buffer, false)) {
			...
			{
				auto metadata = f_metadata("SpeechRecognizer", "Recognize", {
					{"profile", picojson::value("FAR_FIELD")}; //"CLOSE_TALK")},
					{"format", picojson::value("AUDIO_L16_RATE_16000_CHANNELS_1")}
				});
				metadata << "context" & f_context();
				if (v_expecting_dialog_id.empty()) {
					metadata / "event" / "header" << "dialogRequestId" & "dialogRequestId-5" + std::to_string(++v_dialog_id);
				} else {
					metadata / "event" / "header" << "dialogRequestId" & v_expecting_dialog_id;
					v_expecting_dialog_id.clear();
				}
			...
			if (request) {
				...
				while (f_capture(fp, buffer, true)) {
					deque.insert(deque.end(), buffer, buffer + 320);
					if (*p) {
						request->resume();
						// std::cout<<"   resume ===  "<<std::endl;
			...
			fclose(fp);
			v_recognizer->f_wait(std::chrono::milliseconds(10000));
	...
	void f_recognizer() 
	{
		...
		while (true) 
		{
			std::cout<<"----------------------------- start -------------------------------- " << f_online()<<std::endl;

			// if not online
			while (!f_online()) {
				v_recognizer->f_wait(std::chrono::milliseconds((1)));
			}

			while (!v_dialog->v_playing.empty() /*&& 
				!v_content->v_playing.empty()*/ ) {
				v_recognizer->f_wait(std::chrono::milliseconds((1)));
			}
			...
			// FILE *fp = fopen("/mnt/UDISK/req.pcm","r");
			// if (fp == NULL) {
			// 	printf("open file fail\n");
			// 	exit(-1);
			// }
			// std::ifstream fin("/mnt/UDISK/req.pcm", std::ios::binary);
			...
			std::cout<<"=====>>>>> "<<wakeup_ready<<std::endl;

			while (!wakeup_ready || (!v_expecting_timeout && v_expecting_speech)) {
				if (!v_expecting_timeout && v_expecting_speech){
					v_expecting_speech();
					break;
				}
				else{
			...
			if (request) {
				...
				while (wakeup_ready || (rb_data_size(&g_WakeUpRingBuffer) >= 320)) {
					// if (is_StopCapture) {
					// 	is_StopCapture = false;
					// 	break;
					// }
					rlen = rb_read(&g_WakeUpRingBuffer, (unsigned char *) buffer, 320);
					if (rlen <= 20) {
						// v_recognizer->f_wait(std::chrono::milliseconds((1)));
						continue;
					}
					deque.insert(deque.end(), buffer, buffer + rlen);
					if (*p) {
						request->resume();
					}
				}
				is_StopCapture = false;
				// while (!fin.eof()/*!feof(fp)*/) {
				// 	// rlen = fread(buffer,1,320,fp);
				// 	fin.read((char*)buffer, 320);
				// 	rlen = fin.gcount();
				// 	deque.insert(deque.end(), buffer, buffer + rlen);
				// 	if (*p) {
				// 		request->resume();
...
public:
	...
	t_session(t_scheduler& a_scheduler, const std::function<std::ostream&(t_severity)>& a_log, 
			const std::function<std::function<void(bool)>(const std::string&)> a_open_sound) :
			 v_tls(boost::asio::ssl::context::tlsv12), v_scheduler(a_scheduler), v_log(a_log), v_open_sound(a_open_sound)
	{
		// std::cout<<__FUNCTION__<<" "<<__LINE__<<std::endl;
	...
	static void audio_dataReceived(void* userp,int seq,unsigned short* pData,unsigned int length){
		rb_write(&g_WakeUpRingBuffer, (unsigned char*)pData, length * sizeof(unsigned short));
	}

	static void audioStatusChanged(void* userp,int type){
		switch (type) {
	        case WAKEN:
	        std::cout<<"WAKEN\n"<<std::endl;
	        rb_clear(&g_WakeUpRingBuffer);
	        wakeup_ready = 1;
	        break;
	        case VAD_BEGIN:
	        std::cout<<"VAD_BEGIN\n"<<std::endl;
