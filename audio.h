...
class t_audio_decoder
{
	...
	void f_decode(T_target a_target, const AVPacket* a_packet)
	{
		...
		while (true) {
			...
			if (channels > 2) {
				std::cout<<"too many channels: "<< channels<<std::endl;
				channels = 2;
			}
...
class t_audio_target
{
	...
	{
		ALint channels;
		alGetBufferi(a_buffer, AL_CHANNELS, &channels); //channel数，可以大于1，但播放时不能定位
		ALint bits;
		alGetBufferi(a_buffer, AL_BITS, &bits);  //位宽
		ALint size;
		alGetBufferi(a_buffer, AL_SIZE, &size); //总大小(Byte)
		ALint frequency;
		alGetBufferi(a_buffer, AL_FREQUENCY, &frequency); //频率(Hz)
	...
	{
		alSourceUnqueueBuffers(v_source, a_n, a_p); //出队的buffer个数
...
public:
	t_audio_target()
	{
		alGenSources(1, &v_source); //创建1个source
	}
	~t_audio_target()
	{
		f_stop();
	...
	{
		ALint queued = f_get(AL_BUFFERS_QUEUED); //入队列的buffer个数
		ALint n = f_get(AL_BUFFERS_PROCESSED);  //队列中已经被处理的buffer数
		ALuint buffer;
		if (n > 1) {
		...
		alSourceQueueBuffers(v_source, 1, &buffer); //附加一个buffer到一个source上
		v_remain += f_duration(buffer);
		ALint state = f_get(AL_SOURCE_STATE);
		if (state != AL_PLAYING) 
			alSourcePlay(v_source); //播放
		alGetSourcef(v_source, AL_SEC_OFFSET, &v_offset); //播放的位置，秒
		return ++queued - n;
	}
	ALint f_flush()
	{
...
class t_audio_target
{
	double f_duration(unsigned int a_buffer)
	{
		return size * 8.0 / (channels * bits * frequency);
	}

	unsigned int v_source;
	double v_remain = 0.0;
	double v_processed = 0.0;
	float v_offset = 0.0;
	int size;
	...
	void f_unqueue(unsigned int* a_p, size_t a_n)
	{
	}
...
public:
	t_audio_target()
	{
		// char name[32] = {"/tmp/pcm_"};
		// name[9] = fcnt;
		// // sprintf(name, "pcm_%d.pcm", fcnt++);
		// fp = fopen(name, "w");
		// if (fp == NULL) {
	...
	int operator()(size_t a_channels, size_t a_bytes, const char* a_p, size_t a_n, size_t a_rate)
	{
		//a_target(a_channels, sizeof(T_0), data.data(), data.size(), v_codec->sample_rate);
		// size = a_n;
		// channels = a_channels;
		...
		// frequency = a_rate;
		// fwrite(a_p, 1, a_n, fp);
		mw_pcm_play(0,(char*)a_p, (int)a_n, (int)a_rate, (int)a_channels);
		return 33;
	}
